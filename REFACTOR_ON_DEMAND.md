...
**Status (2026-09-24): merged into `fix/audit-findings` together with the
full-audit fixes; the full suite is green.** Sections 2 and 9 below describe the
branch as of 2026-09-17 and are kept as history.

Changes made while merging, on top of this branch:

- `accept_offer` / `decline_offer` lock the booking **and** the offer and
  re-check everything on the locked rows: the booking must still be `PENDING`,
  the visit time must not have passed, and a decline on a reserved booking
  does not cascade to a third contractor.
- Dispatch skips reserved bookings (`ACCEPTED_PENDING_PAYMENT`/`ACCEPTED`),
  never creates a second live offer, excludes suspended accounts, and enforces
  `DISPATCH_MAX_DISTANCE_KM` (50 km) with a straight-line pre-check before the
  directions call. Offers never outlive the scheduled visit.
- `start_charge_for_booking` and `retry_payment` commit the `PROCESSING`
  record first and call the provider **outside** the transaction; a provider
  exception keeps the record (outcome unknown) instead of erasing it.
- `bookings.repair_confirmed_bookings` (every 5 min) starts a missing charge,
  confirms a paid-but-unassigned booking, creates a missing job and releases a
  missing payout.
- The contractor offer inbox is `GET /contractor/offers` (optionally
  `include_closed=true`) plus `GET /contractor/offers/{id}`; the detail shape
  extends `OfferOut`.

When this is documented in [PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md), delete
this file.
